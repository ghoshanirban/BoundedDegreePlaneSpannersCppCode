#ifndef LIBSPANNER_BGS05_H
#define LIBSPANNER_BGS05_H
 ...
 */
            ...
            inline SplitVertex get_s_m(const DelaunayGraph &SG, const SplitVertexSet &V, const SplitVertexEdgeMap &E,
                                       const SplitVertex &v_split) {
                index_t s_i;
 ...
 */
            ...
            void addForwardEdges(const DelaunayGraph &SG,
                ...
                std::vector<SplitVertex> add(subangles, SplitVertex(
                        SG.m_DT.infinite_vertex())); // initialize add to infinite vertex
            ...
            do { // loop through level queue
                ...
                forEachNeighbor(SG, V, E, v_i,
                                [&](const SplitVertex &v_n) {
                                    if (!contains(status,
                                                  v_n.key)) { // If N[v_i] is NOT Known, queue it and add to Known
                                        level.push(v_n.key);
    ...
    void BGS05(const Algorithms::input_t &in, Algorithms::output_t &out) {
