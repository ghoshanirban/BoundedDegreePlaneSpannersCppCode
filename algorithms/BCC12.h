#ifndef LIBSPANNER_BCC12_H
#define LIBSPANNER_BCC12_H
...
//        }
        ...
        getNeighborsInCone(const index_t p, const index_t q, const cone_t cone, const cone_t numCones,
                           const DelaunayL2 &DT, const std::vector<VertexHandle> &handles,
                           const std::vector<index_t> &closest,
...
//            }
            ...
            if (i != j && i + 1 < k &&
                getAngle(handles[Q[i + 1]]->point(), handles[Q[i]]->point(), handles[p]->point()) > PI_OVER_TWO) {
                add.emplace_back(Q[i], Q[i + 1]);
            }

            if (i != k && i > j + 1 &&
                getAngle(handles[p]->point(), handles[Q[i]]->point(), handles[Q[i - 1]]->point()) > PI_OVER_TWO) {
                add.emplace_back(Q[i], Q[i - 1]);
            }
...
//            auto firstInQPrimeIterator = ;
            ...
            if (iPlusOneCw && iPlusOneCcw && f == i + 1) {
                ...
                if (f + 1 != k && theta > FOUR_PI_OVER_SEVEN &&
                    (vertexDegree[Q[i]] < numCones - 1 && vertexDegree[Q[f + 1]] < numCones - 1)) {
                ...
                if (l == k - 1 && (vertexDegree[Q[l]] < numCones - 1 && vertexDegree[Q[b]] < numCones - 1)) {
                    ...
                    if (contains(Q_prime, Q[l - 1]) &&
                        (vertexDegree[Q[l]] < numCones - 1 && vertexDegree[Q[l - 1]] < numCones - 1)) {
...
//            }
    ...
    void BCC12(const Algorithms::input_t &in, Algorithms::output_t &out) {
...
//        };
//        Algorithms_experiment::bcc2012::tikz.drawEdges(DT, tikz.triangulationEdgeOptions);
//
//         Algorithms_experiment::bcc2012::tikz.drawEdges(E.begin(),E.end(),P,edgeOptions);
//
//
//
////        vector<Point> target = {P[TARGET]};
//        Algorithms_experiment::bcc2012::tikz.drawVertices(P.begin(), P.end(), tikz.activeVertexOptions);
//
//
