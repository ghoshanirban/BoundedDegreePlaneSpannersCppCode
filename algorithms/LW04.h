#ifndef LIBSPANNER_LW04_H
#define LIBSPANNER_LW04_H
...
// alpha is set to pi/2
    void LW04(const Algorithms::input_t &in, Algorithms::output_t &out,
...
//        bool noProcessedNeighbors = !isProcessed.at( N->info() );
            ...
            while (--N != done) {
                if ((!T.is_infinite(N) &&
                     isProcessed.at(N->info()))) { // check for v_inf first or isProcessed will be out of range
                ...
                number_t sectorAngle = sectorBoundaries.size() == 1 ?
                                       360.0
                                                                    : getAngle(sectorBoundaries.at(i)->point(),
                                                                               u_handle->point(),
                                                                               sectorBoundaries.at((i + 1) %
                                                                                                   sectorBoundaries.size())->point());
...
//                        cone = 0;
                        ...
                        if (T.is_infinite(closest.at(sector).at(cone))
                            ...
                            closest.at(sector).at(
                                    cone) = N->handle();   // if the saved vertex is infinite or longer than the current one, update
