...
#ifndef LIBSPANNER_KPT17_H
#define LIBSPANNER_KPT17_H
...
namespace BoundedDegreePlaneSpanners {
    ...
    namespace kpt2017 {
        ...
        inline Color getColor(const index_t p, const index_t q, const Algorithms::input_t &H) {
            //cout<<"Getting color of "<<p<<"-"<<q<<endl;
            return td::getCone(p, q, H) % 3 == 1 ? Blue : White;
        }
        inline Color getColor(const Edge &e, const Algorithms::input_t &H) {
        ...
        inline ConePolarity getConePolarity(const index_t p, const index_t q, const Algorithms::input_t &H) {
            return ConePolarity(td::getCone(p, q, H) % 2);
        }

        //Finds the bisector length of a given edge.
        number_t bisectorLength(const Edge &e, const Algorithms::input_t &H) {
...
//                bool same_y = CGAL::compare_y(P[localAnchors[i]], P[w]) == CGAL::EQUAL;
        ...
        void addWhiteAnchors(AnchorList &whiteAnchors, const Algorithms::input_t &P, EdgeList &A) {
...
// Main algorithm.
    void KPT17(const Algorithms::input_t& in, Algorithms::output_t& out) {
