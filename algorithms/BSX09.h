#ifndef LIBSPANNER_BSX09_H
#define LIBSPANNER_BSX09_H
...
void BSX09(const Algorithms::input_t& in, Algorithms::output_t& out,
    ...
    for( index_t u : ordering ) {
        ...
        do { // Find closest unprocessed neighbor, also count processed neighbors and neighbors in ePrime
            if( !T.is_infinite(N) ) {
                if( BoundedDegreePlaneSpanners::contains(ePrime, makeNormalizedPair(u, N->info() ) ) )
                    ++degree;
